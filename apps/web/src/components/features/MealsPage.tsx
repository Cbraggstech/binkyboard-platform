  ...
  return (
    <div className="space-y-6">
      ...
      <div className="flex items-center justify-between">
        <div>
          <h1 className="text-3xl font-bold text-dark">Meal Planner</h1>
          <p className="text-dark/70">Plan healthy meals for your family</p>
        </div>
          ...
          return (
            <div key={mealType} className="card">
              <div className="flex items-center justify-between mb-4">
                <h3 className="text-lg font-semibold text-dark capitalize">{mealType}</h3>
                <UtensilsCrossed size={20} className="text-primary" />
              </div>
              {meal ? (
                <div>
                  <h4 className="font-medium text-dark mb-2">{meal.name}</h4>
      ...
      {/* Weekly Overview */}
      <div className="card">
        <h2 className="text-xl font-semibold text-dark mb-4">This Week's Meals</h2>
        <div className="grid grid-cols-7 gap-2">
          {['Mon', 'Tue', 'Wed', 'Thu', 'Fri', 'Sat', 'Sun'].map((day, i) => (
            <div key={day} className="p-3 bg-white/20 rounded-lg">
              <h4 className="font-medium text-dark mb-2">{day}</h4>
